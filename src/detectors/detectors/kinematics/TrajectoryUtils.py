...
def solve_interception(p_ball_start, v_ball_start, p_ellipsoid_center, axes, eps=0.5):
        """
        Solves line-ellipsoid intersection.

        Arguments:
            p_ball_start (np.array): Starting position of the ball [x, y, z]
            v_ball_start (np.array): Velocity of the ball [vx, vy, vz]
            p_ellipsoid_center (np.array): Center of the ellipsoid [x, y, z]
            axes (tuple or np.array): The semi-axes lengths of the ellipsoid (a, b, c)
            eps (float): Small time offset to avoid missing catches due to approximation
            
        Returns:
            t (float): Time of interception
            p (np.array): Position of interception in world frame
        """
        # 1. Unpack ellipsoid parameters
        a, b, c = axes
        
        # Essentially, transform the problem into "Unit Sphere" space and solve as in sphere case
        ...
        quad_c = np.dot(p_scaled, p_scaled) - 1.0  # Radius is 1.0 in this space
        
        delta = quad_b**2 - 4 * quad_a * quad_c
        
        if delta < 0:
            # CASE: Ball misses ellipsoid.
            t_closest = -quad_b / (2 * quad_a)
            
            if t_closest < 0: 
                return 0.0, p_ball_start # Passed already
                
            # Return closest point in WORLD space
            p_closest = p_ball_start + v_ball_start * t_closest
            return t_closest, p_closest

        # CASE: Ball hits ellipsoid.
