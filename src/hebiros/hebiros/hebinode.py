...
class HebiNode(Node):
    ...
    def __init__(self, name):
        ...
        self.fbkmsg.position     = self.fbkzeros
        self.fbkmsg.velocity     = self.fbkzeros
        self.fbkmsg.effort       = self.fbkzeros
    ...
    def getstringarray(self, name):
        ...
        self.declare_parameter(name, ['unknown'], descriptor=desc)
        parameter  = self.get_parameter(name)
    ...
    def readparameters(self):
        ...
        if len(self.motors) == 0 or self.motors[0] == 'unknown':
            self.error("No motors specified!")
            raise Exception("Inconsistent ROS parameters")
        if len(self.joints) == 0 or self.joints[0] == 'unknown':
    ...
    def cmdCB_sim(self, cmdmsg):
        ...
        self.fbkmsg.position = self.fbkzeros if (lp==0) else cmdmsg.position
        self.fbkmsg.velocity = self.fbkzeros if (lv==0) else cmdmsg.velocity
        self.fbkmsg.effort   = self.fbkzeros if (le==0) else cmdmsg.effort
